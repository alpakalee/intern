...
SERVICE_ACCOUNT_FILE = ''  # 여기에 실제 파일 경로를 넣으세요
...
SPREADSHEET_ID = ''  # 여기에 실제 스프레드시트 ID를 넣으세요
SHEET_NAME = 'Tags'
...
def download_files(search_tag, dest_folder):
    ...
    if not file_names:
        messagebox.showinfo("Info", f"해당하는 파일이 존재하지 않습니다.\n태그 : '{search_tag}'")
        return
    ...
    messagebox.showinfo("Info", f"다운로드 완료\n다운로드 파일 갯수 : {len(file_names)} \n다운로드 위치 : {dest_folder}")
    root.quit()

class App:
    def __init__(self, root):
        self.root = root
        self.root.title("구글드라이브 사진 다운로더")

        self.tag_label = tk.Label(root, text="다운로드 할 사진의 태그 :")
        self.tag_label.pack(padx=10, pady=5)
        
        self.tag_entry = tk.Entry(root, width=50)
        self.tag_entry.pack(padx=10, pady=5)
        
        self.folder_label = tk.Label(root, text="다운로드 할 위치(폴더경로):")
        self.folder_label.pack(padx=10, pady=5)
        
        self.folder_entry = tk.Entry(root, width=50)
        self.folder_entry.pack(padx=10, pady=5)
        
        self.browse_button = tk.Button(root, text="폴더 찾기", command=self.browse_folder)
        self.browse_button.pack(pady=5)

        self.download_button = tk.Button(root, text="다운로드", command=self.download)
    ...
    def download(self):
        ...
        if not dest_folder:
            messagebox.showwarning("Warning", "다운로드 할 폴더를 입력해주세요")
            return
